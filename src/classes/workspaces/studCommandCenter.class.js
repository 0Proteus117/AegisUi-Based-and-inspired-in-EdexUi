...
class StudCommandCenter {
    ...
    renderAssignmentDetail(context) {
        ...
        return `<div class="stud-detail-heading"><small>ASSIGNMENT</small><h3>${this.escape(assignment.title)}</h3><span>${this.escape(course ? this.courseLabel(course.id) : "NO MODULE")} · ${this.escape(assignment.status)}</span></div>
            ...
            <section class="stud-object-section stud-assignment-roadmap"><header><h3>ASSIGNMENT ROADMAP</h3><span>${this.state.workflowAssignmentId === assignment.id ? "WORKFLOW TEST SELECTED" : "EXPLICIT / LOCAL"}</span></header><ol class="stud-roadmap-steps"><li><strong>UNDERSTAND BRIEF</strong><small>${assignment.description ? "DESCRIPTION AVAILABLE" : "BRIEF NOT YET IDENTIFIED"}</small></li><li><strong>REQUIREMENTS</strong><small>${requirements.length ? `${requirements.length} TRACEABLE OBSERVATIONS` : "NO REQUIREMENTS EXTRACTED"}</small></li><li><strong>COURSE MATERIAL</strong><small>${resources.length ? `${resources.length} RELATED RESOURCES` : "ADD OR SYNC MATERIAL"}</small></li><li><strong>RESEARCH</strong><small>${papers.length ? `${papers.length} PAPERS LINKED` : "NO PAPERS LINKED"}</small></li><li><strong>NOTES &amp; OUTLINE</strong><small>${notes.length ? `${notes.length} LOCAL NOTES` : "NO LOCAL NOTES"}</small></li><li><strong>DRAFT</strong><small>USER-CONTROLLED · NO HIDDEN AUTOSAVE</small></li><li><strong>CITATIONS</strong><small>${papers.length ? "SOURCE RECORDS AVAILABLE" : "SOURCE RECORDS NEEDED"}</small></li><li><strong>REVIEW</strong><small>${revisions.length ? `${revisions.length} REVISION ITEMS` : "RUBRIC / REVIEW NOT YET RECORDED"}</small></li><li><strong>FINAL</strong><small>${["SUBMITTED", "GRADED"].includes(assignment.status) ? assignment.status : `${assignment.localProgress ?? 0}% LOCAL PROGRESS`}</small></li></ol><div class="stud-detail-actions"><button type="button" data-stud-assignment-knowledge="${this.escape(assignment.id)}">BUILD ACADEMIC CONTEXT</button><button type="button" data-stud-assignment-research="${this.escape(assignment.id)}">CONTINUE RESEARCH</button><button type="button" data-stud-dialog="CREATE_NOTE">TAKE NOTE</button><button type="button" data-stud-workflow-assignment="${this.escape(assignment.id)}">${this.state.workflowAssignmentId === assignment.id ? "WORKFLOW TEST SELECTED" : "SELECT FOR WORKFLOW TEST"}</button></div></section>
