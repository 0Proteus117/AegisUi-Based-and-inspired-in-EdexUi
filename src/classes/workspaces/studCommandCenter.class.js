...
const ACTIVE_VIEWS = Object.freeze(["OVERVIEW", "MODULES", "ASSIGNMENTS", "RESEARCH", "NOTES", "SERVICES", "MOODLE"]);
...
class StudCommandCenter {
    ...
    mount(view) {
        ...
        this.bind();
        return Promise.all([this.research.initialize(), this.moodle.initialize(), this.refresh()]).then(() => this.render());
    }
    ...
    async loadAssignmentContext(id) {
        const [assignment, provenance, relationships, references, resources] = await Promise.all([
            this.request("stud-entity-read", {entityType: "ASSIGNMENT", entityId: id}),
            this.request("stud-provenance-list", {entityType: "ASSIGNMENT", entityId: id}),
            this.request("stud-relationship-list", {entityType: "ASSIGNMENT", entityId: id}),
            this.request("stud-reference-list", {entityType: "ASSIGNMENT", entityId: id}),
            this.request("stud-entity-list", {entityType: "RESOURCE", assignmentId: id, limit: 100})
        ]);
        this.state.assignmentContext = {assignment, provenance, relationships, references, resources};
    }
    ...
    renderOverview() {
        ...
        return `<section class="stud-overview-grid">
            ...
            <article class="workspace-panel stud-overview-context"><header><h2>ACADEMIC CONTEXT</h2><span>OFFLINE / LOCAL FIRST</span></header><div class="workspace-panel-content"><p>STUD derives Today, Upcoming, Priority and Continue only from canonical local records. Calendar and Email become visible only after an explicit reference is linked.</p><button type="button" data-stud-nav-view="MODULES">MANAGE MODULES</button><button type="button" data-stud-nav-view="ASSIGNMENTS">MANAGE ASSIGNMENTS</button></div></article>
        </section>`;
    }
    ...
    renderModuleDetail(context) {
        const {course, assignments, resources, notes, papers, references, provenance} = context;
        ...
        return `<div class="stud-detail-heading"><small>MODULE</small><h3>${this.escape(course.title)}</h3><span>${this.escape(course.code || "NO CODE")} · ${this.escape(course.status)}</span>${course.startDate || course.endDate ? `<small>${course.startDate ? dateText(course.startDate) : "START UNKNOWN"} → ${course.endDate ? dateText(course.endDate) : "END UNKNOWN"}</small>` : ""}</div>
            <div class="stud-detail-actions"><button type="button" data-stud-dialog="EDIT_MODULE">EDIT MODULE</button><button type="button" data-stud-dialog="CREATE_ASSIGNMENT">NEW ASSIGNMENT</button><button type="button" data-stud-dialog="CREATE_NOTE">NEW NOTE</button><button type="button" data-stud-dialog="ADD_RESOURCE">ADD RESOURCE</button></div>
            <div class="stud-object-columns">${objects("ASSIGNMENTS", assignments, "ASSIGNMENT", "NO ASSIGNMENTS STORED IN STUD.")}${objects("RESOURCES", resources, "RESOURCE", "NO LOCAL RESOURCES.")}${objects("NOTES", notes, "NOTE", "NO LOCAL NOTES.")}${objects("PAPERS", papers, "RESEARCH_PAPER", "NO RELATED PAPERS.")}</div>
    ...
    openDialog(kind, trigger) {
        ...
        const template = {
            ...
            LINK_CALENDAR: assignment || course ? ["LINK CALENDAR REFERENCE", `<form class="stud-dialog-form" data-stud-form="LINK_REFERENCE"><input type="hidden" name="kind" value="CALENDAR"><p>Store one explicit local Calendar identifier only. STUD cannot inspect or modify the Calendar event.</p><label>EVENT REFERENCE<input class="aegis-input" name="externalId" maxlength="260" required autofocus></label><button type="submit">LINK REFERENCE</button></form>`] : null,
            LINK_EMAIL: assignment || course ? ["LINK EMAIL REFERENCE", `<form class="stud-dialog-form" data-stud-form="LINK_REFERENCE"><input type="hidden" name="kind" value="EMAIL"><p>Store one explicit local message identifier only. STUD cannot inspect, search or copy email content.</p><label>MESSAGE REFERENCE<input class="aegis-input" name="externalId" maxlength="260" required autofocus></label><button type="submit">LINK REFERENCE</button></form>`] : null
        }[kind];
    ...
    bind() {
        ...
        this.view.addEventListener("submit", async event => { if (await this.research.handleSubmit(event)) return; if (await this.moodle.handleSubmit(event)) return; this.handleForm(event); });
        const dialog = this.view.querySelector("[data-stud-dialog-element]");
