...
class StudMissionControlWorkspace {
    ...
    reset() { clearTimeout(this.refreshTimer);this.state = {...this.state, mission: null, events: [], runArtifacts: [], selectedPlanId:"",selectedStepId:"",selectedRunId: "", selectedArtifactId: "", relationships: [], error: ""}; }
    ...
    async load() {
        ...
        try {
            this.state.mission = await this.request("stud-execution-state", {assignmentId: assignment.id, ...(this.state.selectedPlanId?{planId:this.state.selectedPlanId}:{}),eventLimit: 100, artifactLimit: 50});
            if (this.state.mission.executionPlan && this.state.mission.executionPlan.parentRunId) this.state.selectedRunId = this.state.mission.executionPlan.parentRunId;
            ...
            if (this.state.selectedRunId) [this.state.events, this.state.runArtifacts] = await Promise.all([
                Promise.resolve(this.state.mission.events || []).then(events => events.length ? events : this.request("stud-operation-events", {assignmentId: assignment.id, runId: this.state.selectedRunId, limit: 100})),
    ...
    scheduleRefresh(){
        ...
        if(!this.state.mission||!this.state.mission.executionPlan||this.state.mission.executionPlan.state!=="RUNNING")return;
    ...
    renderMission() {
        ...
        if (mission.executionPlan || (!mission.activeRuns.length && !mission.recentRuns.length && Object.prototype.hasOwnProperty.call(mission,"executionPlan"))) return this.renderExecutionMission(mission);
