...
class StudResearchWorkspace {
    ...
    renderNotes() {
        ...
        return `<section class="stud-notes-shell"><header class="stud-section-title"><div><small>STUD / NOTES</small><h2>STRUCTURED ACADEMIC NOTES</h2><p>Local ProseMirror-compatible document model with explicit paper, module and assignment relationships.</p></div><span>LOCAL / EXPLICIT SAVE</span></header><div class="stud-notes-grid"><article class="workspace-panel"><header><h2>NOTES</h2><span>${this.state.notes.length} LOCAL</span></header><div class="workspace-panel-content"><button type="button" data-stud-new-note>NEW NOTE</button><div class="stud-note-list">${this.state.notes.map(item => `<button type="button" class="${item.id === this.state.selectedNoteId ? "selected" : ""}" data-stud-note-id="${this.escape(item.id)}"><strong>${this.escape(item.title)}</strong><small>${this.escape(item.content || "EMPTY NOTE")}</small></button>`).join("") || `<div class="stud-empty-inline">NO NOTES YET.</div>`}</div></div></article><article class="workspace-panel stud-note-editor-panel"><header><h2>NOTE EDITOR</h2><span>BROWSER STRUCTURED / JSON V1</span></header><div class="workspace-panel-content">${this.renderNoteEditor({note, selectedPaperId: this.state.notePaperId || ""})}</div></article></div></section>`;
    }

    renderContextualNoteEditor({note = null, course = null, assignment = null, selectedPaperId = ""} = {}) {
        return this.renderNoteEditor({note, course, assignment, selectedPaperId, contextual: true});
    }

    renderNoteEditor({note = null, course = null, assignment = null, selectedPaperId = "", contextual = false} = {}) {
        ...
        const selectedCourseId = note && note.courseId || course && course.id || "";
        const selectedAssignmentId = note && note.assignmentId || assignment && assignment.id || "";
        const contextFields = contextual
            ? `<input type="hidden" name="courseId" value="${this.escape(selectedCourseId)}"><input type="hidden" name="assignmentId" value="${this.escape(selectedAssignmentId)}"><p class="stud-contextual-note-link">${this.escape(course && (course.code || course.title) || "LOCAL COURSE")} · ${this.escape(assignment && assignment.title || "ASSIGNMENT CONTEXT")}</p>`
            : `<label>MODULE<select class="aegis-select" name="courseId"><option value="">NO MODULE</option>${this.parent.state.courses.map(item => `<option value="${this.escape(item.id)}"${selectedCourseId === item.id ? " selected" : ""}>${this.escape(item.code || item.title)}</option>`).join("")}</select></label><label>ASSIGNMENT<select class="aegis-select" name="assignmentId"><option value="">NO ASSIGNMENT</option>${this.parent.state.assignments.map(item => `<option value="${this.escape(item.id)}"${selectedAssignmentId === item.id ? " selected" : ""}>${this.escape(item.title)}</option>`).join("")}</select></label>`;
        return `<form data-stud-research-form="NOTE" data-stud-note-id="${this.escape(note && note.id || "")}" class="stud-note-form${contextual ? " is-contextual" : ""}"><div class="stud-editor-fields"><label>TITLE<input class="aegis-input" name="title" maxlength="240" required value="${this.escape(note && note.title || (this.state.noteSelection ? "Quoted document observation" : "New academic note"))}"></label>${contextFields}<label>PAPER<select class="aegis-select" name="paperId"><option value="">NO PAPER LINK</option>${this.state.library.map(item => `<option value="${this.escape(item.id)}"${selectedPaperId === item.id ? " selected" : ""}>${this.escape(item.title)}</option>`).join("")}</select></label></div><div class="stud-note-toolbar" role="toolbar" aria-label="Note formatting"><button type="button" data-stud-editor-command="bold">BOLD</button><button type="button" data-stud-editor-command="italic">ITALIC</button><button type="button" data-stud-editor-command="heading">HEADING</button><button type="button" data-stud-editor-command="bulletList">LIST</button><button type="button" data-stud-editor-command="blockquote">QUOTE</button><button type="button" data-stud-editor-command="codeBlock">CODE</button><button type="button" data-stud-editor-command="link">LINK</button><button type="button" data-stud-editor-command="inlineMath">MATH</button><button type="button" data-stud-editor-command="blockMath">DISPLAY MATH</button><button type="button" data-stud-editor-command="table">TABLE</button><button type="button" data-stud-insert-citation>CITATION</button></div>${this.state.noteSelection ? `<div class="stud-selection-provenance">QUOTE · PAGE ${this.state.noteSelection.page} · SHA-256 ${this.escape(this.state.noteSelection.selectionTextHash.slice(0, 16))}… · SOURCE TEXT REMAINS DISTINCT FROM STUDENT NOTE</div>` : ""}<div class="stud-tiptap-editor" data-stud-editor aria-label="Structured note editor"></div><script type="application/json" data-stud-note-document>${this.scriptJson(document)}</script><div class="stud-detail-actions"><button type="submit">SAVE STRUCTURED NOTE</button>${note ? `<button type="button" data-stud-note-revision="${this.escape(note.id)}">CREATE REVISION</button>` : ""}</div></form>`;
    }
    ...
    async afterRender(view) {
        if (view === "NOTES" || (view === "ASSIGNMENTS" && this.parent.assignmentWorkspace && this.parent.assignmentWorkspace.shouldMountNoteEditor())) await this.mountEditor();
    }
    ...
    async saveNote(form, values) {
        ...
        this.state.selectedNoteId = note.id; this.state.notePaperId = ""; this.state.noteSelection = null; await this.refreshLibrary();
        if (this.parent.assignmentWorkspace && this.parent.state.activeView === "ASSIGNMENTS") await this.parent.assignmentWorkspace.onNoteSaved(note);
        this.parent.render(); this.toast(this.parent.view, "STRUCTURED NOTE SAVED LOCALLY");
    }
