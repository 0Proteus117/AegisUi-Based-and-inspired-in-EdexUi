...
class StudAssignmentWorkspace {
    ...
    renderActions() {
        const active = this.activeObject();
        return `<footer class="stud-assignment-workspace-actions"><div><small>CONTEXTUAL ACTIONS</small><span>${active ? `${this.escape(objectTypeLabel(active.entityType))} · ${this.escape(objectTitle(active))}` : "SELECT A RELATED OBJECT"}</span></div><div><button type="button" data-stud-workspace-mode="COMPOSITION">COMPOSITION</button><button type="button" data-stud-workspace-mode="HUMANISATION">REFINE STYLE</button><button type="button" data-stud-workspace-mode="LECTURER_REVIEW">ACADEMIC REVIEW</button><button type="button" data-stud-workspace-mode="RESEARCH_PLAN">RESEARCH PLAN</button><button type="button" data-stud-workspace-mode="FACULTY_SCOUT">FACULTY SCOUT</button><button type="button" data-stud-workspace-mode="EVIDENCE_MAP">EVIDENCE MAP</button><button type="button" data-stud-workspace-mode="ARTIFACTS">ARTIFACTS</button><button type="button" data-stud-workspace-mode="MISSION">ACTIVITY</button><button type="button" data-stud-workspace-open-specialist="RESEARCH">RESEARCH LIBRARY</button><button type="button" data-stud-workspace-open-specialist="KNOWLEDGE">KNOWLEDGE</button><button type="button" data-stud-workspace-open-specialist="CITATIONS">CITATIONS</button><button type="button" data-stud-workspace-open-specialist="WORKBENCH">WORKBENCH</button></div></footer>`;
    }
    ...
    renderWorkspace() {
        ...
        if (this.state.mode === "LECTURER_REVIEW") return `<section class="stud-assignment-workspace-detail is-lecturer-review"><header><button type="button" data-stud-workspace-mode="COMPOSITION">← COMPOSITION</button><div><small>LECTURER COMMITTEE / CORRECTIONS</small><h2>${this.escape(assignment.title)}</h2></div></header>${this.lecturerCommittee.render()}</section>`;
    ...
    async handleClick(event) {
        ...
        if (await this.lecturerCommittee.handleClick(event)) return true;
    ...
    async handleChange(event) { if(await this.humanisation.handleChange(event))return true;return this.lecturerCommittee.handleChange(event); }
}
