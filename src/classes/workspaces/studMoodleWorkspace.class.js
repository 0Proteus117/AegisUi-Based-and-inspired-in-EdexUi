...
class StudMoodleWorkspace {
    ...
    startAuthPolling() {
        ...
        const poll = async () => {
            ...
            if (provider.tokenConfigured || !provider.authenticationPending || Date.now() >= expiresAt) {
                ...
                if (provider.tokenConfigured) { await this.parent.documents.refresh(); await this.parent.refresh(); }
            }
            this.parent.render();
        };
    ...
    async handleClick(event) {
        ...
        if (kind === "connect" || kind === "open-web" || kind === "reauthenticate") { try { await this.request("stud-moodle-open-web", {}); await this.refreshStatus(); this.state.error = ""; this.startAuthPolling(); this.parent.render(); } catch (error) { this.state.error = error.message; this.parent.render(); } return true; }
