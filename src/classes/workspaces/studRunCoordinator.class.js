...
class StudRunCoordinator{
    ...
    operationalHistory(plan,eventLimit,artifactLimit){
        ...
        return {events:Object.freeze(events),planArtifacts:Object.freeze(planArtifacts),historyBounds:{runLimit:512,eventLimit,artifactLimit,runBoundReached:runCount>=512,truncated:runCount>=512||eventRows.length>eventLimit||artifactRows.length>artifactLimit}};
    }
    missionState(input={}){
        ...
        if(!plan)return Object.freeze({...m6,executionPlan:null,executionHistory,launchOptions,planArtifacts:Object.freeze([]),resting:true,resource:this.resources.snapshot(),profiles:this.repository.profiles()});
        ...
        return Object.freeze({...m6,executionPlan:Object.freeze({...hydrated,steps}),executionHistory,launchOptions,planArtifacts,historyBounds,incidents,events,resource:this.resources.snapshot(),profiles:this.repository.profiles(),resting:!["RUNNING","PAUSED","WAITING_HUMAN","WAITING_EXTERNAL","INTERRUPTED"].includes(plan.state)});
    }
