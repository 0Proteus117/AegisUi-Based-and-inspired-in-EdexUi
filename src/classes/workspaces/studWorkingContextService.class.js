...
class StudWorkingContextService {
    ...
    assertPayload(input = {}) {
        Model.assertAllowedKeys(input, ["courseId", "assignmentId", "objectType", "objectId", "workflowId", "workflowNodeId", "researchPlanId", "researchTopicId", "claimId", "evidenceId", "compositionPlanId", "compositionSectionId", "draftDocumentId", "draftVersionId", "humanisationProfileId", "humanisationSessionId", "lecturerReviewSessionId", "lecturerFindingId", "correctionPlanId", "correctionItemId", "correctionSessionId", "originSurface", "userPinned"], "Working context");
        return input;
    }
    ...
    validate(input = {}) {
        ...
        const compositionPlanId = input.compositionPlanId ? Model.safeId(input.compositionPlanId, "Composition Plan ID") : null;
        let compositionPlan = null;
        if (compositionPlanId) {
            if (!assignmentId) throw new Model.StudError("INVALID_CONTEXT", "A Composition Plan context requires an active Assignment.");
            compositionPlan = this.db.prepare("SELECT id,assignment_id,research_plan_id,lifecycle,revision,row_version FROM stud_composition_plans WHERE id=?").get(compositionPlanId);
            if (!compositionPlan || compositionPlan.assignment_id !== assignmentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Composition Plan does not belong to the active Assignment.");
            if (researchPlanId && compositionPlan.research_plan_id && compositionPlan.research_plan_id !== researchPlanId) throw new Model.StudError("INVALID_CONTEXT", "The selected Composition Plan references a different Research Plan.");
        }
        const compositionSectionId = input.compositionSectionId ? Model.safeId(input.compositionSectionId, "Composition Section ID") : null;
        let compositionSection = null;
        if (compositionSectionId) {
            if (!compositionPlanId) throw new Model.StudError("INVALID_CONTEXT", "A Composition Section context requires an active Composition Plan.");
            compositionSection = this.db.prepare("SELECT id,plan_id,assignment_id,title,section_order,row_version FROM stud_composition_sections WHERE id=?").get(compositionSectionId);
            if (!compositionSection || compositionSection.plan_id !== compositionPlanId || compositionSection.assignment_id !== assignmentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Section does not belong to the active Composition Plan.");
        }
        const draftDocumentId = input.draftDocumentId ? Model.safeId(input.draftDocumentId, "Draft Document ID") : null;
        let draftDocument = null;
        if (draftDocumentId) {
            if (!compositionPlanId) throw new Model.StudError("INVALID_CONTEXT", "A Draft Document context requires an active Composition Plan.");
            draftDocument = this.db.prepare("SELECT id,assignment_id,composition_plan_id,title,current_version_id,row_version FROM stud_draft_documents WHERE id=?").get(draftDocumentId);
            if (!draftDocument || draftDocument.assignment_id !== assignmentId || draftDocument.composition_plan_id !== compositionPlanId) throw new Model.StudError("INVALID_CONTEXT", "The selected Draft does not belong to the active Assignment and Composition Plan.");
        }
        const draftVersionId = input.draftVersionId ? Model.safeId(input.draftVersionId, "Draft Version ID") : null;
        let draftVersion = null;
        if (draftVersionId) {
            if (!draftDocumentId) throw new Model.StudError("INVALID_CONTEXT", "A Draft Version context requires an active Draft Document.");
            draftVersion = this.db.prepare("SELECT id,draft_id,assignment_id,version_number,origin,created_at FROM stud_draft_versions WHERE id=?").get(draftVersionId);
            if (!draftVersion || draftVersion.draft_id !== draftDocumentId || draftVersion.assignment_id !== assignmentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Draft Version does not belong to the active Draft.");
        }
        const humanisationProfileId = input.humanisationProfileId ? Model.safeId(input.humanisationProfileId, "Humanisation Profile ID") : null;
        let humanisationProfile = null;
        if (humanisationProfileId) {
            humanisationProfile = this.db.prepare("SELECT id,name,genre,lifecycle,current_revision_id,row_version FROM stud_humanisation_profiles WHERE id=?").get(humanisationProfileId);
            if (!humanisationProfile) throw new Model.StudError("INVALID_CONTEXT", "The selected Humanisation Profile does not exist.");
        }
        const humanisationSessionId = input.humanisationSessionId ? Model.safeId(input.humanisationSessionId, "Humanisation Session ID") : null;
        let humanisationSession = null;
        if (humanisationSessionId) {
            if (!assignmentId || !draftDocumentId) throw new Model.StudError("INVALID_CONTEXT", "A Humanisation Session context requires an active Assignment and Draft.");
            humanisationSession = this.db.prepare("SELECT id,assignment_id,draft_id,source_draft_version_id,profile_id,state,row_version,updated_at FROM stud_humanisation_sessions WHERE id=?").get(humanisationSessionId);
            if (!humanisationSession || humanisationSession.assignment_id !== assignmentId || humanisationSession.draft_id !== draftDocumentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Humanisation Session does not belong to the active Assignment and Draft.");
            if (humanisationProfileId && humanisationSession.profile_id !== humanisationProfileId) throw new Model.StudError("INVALID_CONTEXT", "The selected Humanisation Session used a different Profile.");
        }
        const lecturerReviewSessionId = input.lecturerReviewSessionId ? Model.safeId(input.lecturerReviewSessionId, "Lecturer Review Session ID") : null;
        let lecturerReviewSession = null;
        if (lecturerReviewSessionId) {
            if (!assignmentId || !draftDocumentId) throw new Model.StudError("INVALID_CONTEXT", "A Lecturer Review Session requires an active Assignment and Draft.");
            lecturerReviewSession = this.db.prepare("SELECT id,assignment_id,draft_id,source_draft_version_id,state,row_version,updated_at FROM stud_lecturer_review_sessions WHERE id=?").get(lecturerReviewSessionId);
            if (!lecturerReviewSession || lecturerReviewSession.assignment_id !== assignmentId || lecturerReviewSession.draft_id !== draftDocumentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Lecturer Review Session does not belong to the active Assignment and Draft.");
        }
        const lecturerFindingId = input.lecturerFindingId ? Model.safeId(input.lecturerFindingId, "Lecturer Finding ID") : null;
        let lecturerFinding = null;
        if (lecturerFindingId) {
            if (!lecturerReviewSessionId) throw new Model.StudError("INVALID_CONTEXT", "A Lecturer Finding requires an active Review Session.");
            lecturerFinding = this.db.prepare("SELECT id,session_id,assignment_id,category,severity,status,title,row_version,updated_at FROM stud_lecturer_review_findings WHERE id=?").get(lecturerFindingId);
            if (!lecturerFinding || lecturerFinding.session_id !== lecturerReviewSessionId || lecturerFinding.assignment_id !== assignmentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Finding does not belong to the active Review Session.");
        }
        const correctionPlanId = input.correctionPlanId ? Model.safeId(input.correctionPlanId, "Correction Plan ID") : null;
        let correctionPlan = null;
        if (correctionPlanId) {
            if (!assignmentId || !lecturerReviewSessionId) throw new Model.StudError("INVALID_CONTEXT", "A Correction Plan requires an active Assignment and Review Session.");
            correctionPlan = this.db.prepare("SELECT id,assignment_id,review_session_id,source_draft_version_id,title,state,row_version,updated_at FROM stud_correction_plans WHERE id=?").get(correctionPlanId);
            if (!correctionPlan || correctionPlan.assignment_id !== assignmentId || correctionPlan.review_session_id !== lecturerReviewSessionId) throw new Model.StudError("INVALID_CONTEXT", "The selected Correction Plan does not belong to the active Review Session.");
        }
        const correctionItemId = input.correctionItemId ? Model.safeId(input.correctionItemId, "Correction Item ID") : null;
        let correctionItem = null;
        if (correctionItemId) {
            if (!correctionPlanId) throw new Model.StudError("INVALID_CONTEXT", "A Correction Item requires an active Correction Plan.");
            correctionItem = this.db.prepare("SELECT id,plan_id,assignment_id,title,action_type,state,row_version,updated_at FROM stud_correction_items WHERE id=?").get(correctionItemId);
            if (!correctionItem || correctionItem.plan_id !== correctionPlanId || correctionItem.assignment_id !== assignmentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Correction Item does not belong to the active Correction Plan.");
        }
        const correctionSessionId = input.correctionSessionId ? Model.safeId(input.correctionSessionId, "Correction Session ID") : null;
        let correctionSession = null;
        if (correctionSessionId) {
            if (!correctionPlanId) throw new Model.StudError("INVALID_CONTEXT", "A Correction Session requires an active Correction Plan.");
            correctionSession = this.db.prepare("SELECT id,assignment_id,correction_plan_id,source_draft_version_id,state,row_version,updated_at FROM stud_correction_sessions WHERE id=?").get(correctionSessionId);
            if (!correctionSession || correctionSession.correction_plan_id !== correctionPlanId || correctionSession.assignment_id !== assignmentId) throw new Model.StudError("INVALID_CONTEXT", "The selected Correction Session does not belong to the active Correction Plan.");
        }
        ...
        return {course, courseId, assignment, assignmentId, object, objectType, objectId, objectScope, workflow, workflowId, workflowNode, workflowNodeId, researchPlan, researchPlanId, researchTopic, researchTopicId, claim, claimId, evidence, evidenceId, compositionPlan, compositionPlanId, compositionSection, compositionSectionId, draftDocument, draftDocumentId, draftVersion, draftVersionId, humanisationProfile, humanisationProfileId, humanisationSession, humanisationSessionId, lecturerReviewSession, lecturerReviewSessionId, lecturerFinding, lecturerFindingId, correctionPlan, correctionPlanId, correctionItem, correctionItemId, correctionSession, correctionSessionId, originSurface, userPinned: input.userPinned === true};
    }

    hydrate(row) {
        if (!row) return Object.freeze({status: "EMPTY", activeCourse: null, activeAssignment: null, activeRequirementContract: null, activeObject: null, activeWorkflow: null, activeWorkflowNode: null, activeResearchPlan: null, activeResearchTopic: null, activeClaim: null, activeEvidence: null, activeCompositionPlan: null, activeCompositionSection: null, activeDraftDocument: null, activeDraftVersion: null, activeHumanisationProfile: null, activeHumanisationSession: null, activeLecturerReviewSession: null, activeLecturerFinding: null, activeCorrectionPlan: null, activeCorrectionItem: null, activeCorrectionSession: null, originSurface: null, userPinned: false, updatedAt: null});
        ...
        const compositionPlan = row.active_composition_plan_id ? this.db.prepare("SELECT id,assignment_id,research_plan_id,lifecycle,revision,row_version FROM stud_composition_plans WHERE id=?").get(row.active_composition_plan_id) : null;
        const compositionSection = row.active_composition_section_id ? this.db.prepare("SELECT id,plan_id,assignment_id,title,section_order,row_version FROM stud_composition_sections WHERE id=?").get(row.active_composition_section_id) : null;
        const draftDocument = row.active_draft_document_id ? this.db.prepare("SELECT id,assignment_id,composition_plan_id,title,current_version_id,row_version FROM stud_draft_documents WHERE id=?").get(row.active_draft_document_id) : null;
        const draftVersion = row.active_draft_version_id ? this.db.prepare("SELECT id,draft_id,assignment_id,version_number,origin,created_at FROM stud_draft_versions WHERE id=?").get(row.active_draft_version_id) : null;
        const humanisationProfile = row.active_humanisation_profile_id ? this.db.prepare("SELECT id,name,genre,lifecycle,current_revision_id,row_version FROM stud_humanisation_profiles WHERE id=?").get(row.active_humanisation_profile_id) : null;
        const humanisationSession = row.active_humanisation_session_id ? this.db.prepare("SELECT id,assignment_id,draft_id,source_draft_version_id,profile_id,state,row_version,updated_at FROM stud_humanisation_sessions WHERE id=?").get(row.active_humanisation_session_id) : null;
        const lecturerReviewSession = row.active_lecturer_review_session_id ? this.db.prepare("SELECT id,assignment_id,draft_id,source_draft_version_id,state,row_version,updated_at FROM stud_lecturer_review_sessions WHERE id=?").get(row.active_lecturer_review_session_id) : null;
        const lecturerFinding = row.active_lecturer_finding_id ? this.db.prepare("SELECT id,session_id,assignment_id,category,severity,status,title,row_version,updated_at FROM stud_lecturer_review_findings WHERE id=?").get(row.active_lecturer_finding_id) : null;
        const correctionPlan = row.active_correction_plan_id ? this.db.prepare("SELECT id,assignment_id,review_session_id,source_draft_version_id,title,state,row_version,updated_at FROM stud_correction_plans WHERE id=?").get(row.active_correction_plan_id) : null;
        const correctionItem = row.active_correction_item_id ? this.db.prepare("SELECT id,plan_id,assignment_id,title,action_type,state,row_version,updated_at FROM stud_correction_items WHERE id=?").get(row.active_correction_item_id) : null;
        const correctionSession = row.active_correction_session_id ? this.db.prepare("SELECT id,assignment_id,correction_plan_id,source_draft_version_id,state,row_version,updated_at FROM stud_correction_sessions WHERE id=?").get(row.active_correction_session_id) : null;
        if ((row.active_course_id && !course) || (row.active_assignment_id && !assignment) || (row.active_object_id && !object) ||
            ...
            (row.active_composition_plan_id && (!compositionPlan || compositionPlan.assignment_id !== row.active_assignment_id || (row.active_research_plan_id && compositionPlan.research_plan_id && compositionPlan.research_plan_id !== row.active_research_plan_id))) ||
            (row.active_composition_section_id && (!compositionSection || compositionSection.plan_id !== row.active_composition_plan_id || compositionSection.assignment_id !== row.active_assignment_id)) ||
            (row.active_draft_document_id && (!draftDocument || draftDocument.assignment_id !== row.active_assignment_id || draftDocument.composition_plan_id !== row.active_composition_plan_id)) ||
            (row.active_draft_version_id && (!draftVersion || draftVersion.assignment_id !== row.active_assignment_id || draftVersion.draft_id !== row.active_draft_document_id)) ||
            (row.active_humanisation_profile_id && !humanisationProfile) ||
            (row.active_humanisation_session_id && (!humanisationSession || humanisationSession.assignment_id !== row.active_assignment_id || humanisationSession.draft_id !== row.active_draft_document_id || (row.active_humanisation_profile_id && humanisationSession.profile_id !== row.active_humanisation_profile_id))) ||
            (row.active_lecturer_review_session_id && (!lecturerReviewSession || lecturerReviewSession.assignment_id !== row.active_assignment_id || lecturerReviewSession.draft_id !== row.active_draft_document_id)) ||
            (row.active_lecturer_finding_id && (!lecturerFinding || lecturerFinding.session_id !== row.active_lecturer_review_session_id || lecturerFinding.assignment_id !== row.active_assignment_id)) ||
            (row.active_correction_plan_id && (!correctionPlan || correctionPlan.assignment_id !== row.active_assignment_id || correctionPlan.review_session_id !== row.active_lecturer_review_session_id)) ||
            (row.active_correction_item_id && (!correctionItem || correctionItem.assignment_id !== row.active_assignment_id || correctionItem.plan_id !== row.active_correction_plan_id)) ||
            (row.active_correction_session_id && (!correctionSession || correctionSession.assignment_id !== row.active_assignment_id || correctionSession.correction_plan_id !== row.active_correction_plan_id))) {
        ...
        return Object.freeze({status: "READY", activeCourse: course, activeAssignment: assignment, activeRequirementContract: contract ? Object.freeze({id: contract.id, revision: contract.revision, lifecycle: contract.lifecycle, completeness: contract.completeness}) : null, activeObject: object ? Object.freeze({...object, entityType: row.active_object_type}) : null, activeWorkflow: workflow ? Object.freeze({id: workflow.id, assignmentId: workflow.assignment_id, templateVersionId: workflow.template_version_id, rowVersion: workflow.row_version}) : null, activeWorkflowNode: workflowNode ? Object.freeze({id: workflowNode.id, workflowId: workflowNode.workflow_id, title: workflowNode.title, semanticType: workflowNode.semantic_type, state: workflowNode.state, rowVersion: workflowNode.row_version}) : null, activeResearchPlan: researchPlan ? Object.freeze({id:researchPlan.id,assignmentId:researchPlan.assignment_id,workflowId:researchPlan.workflow_id,lifecycle:researchPlan.lifecycle,revision:researchPlan.revision,rowVersion:researchPlan.row_version}) : null, activeResearchTopic: researchTopic ? Object.freeze({id:researchTopic.id,planId:researchTopic.plan_id,assignmentId:researchTopic.assignment_id,title:researchTopic.title,disposition:researchTopic.disposition,rowVersion:researchTopic.row_version}) : null, activeClaim: claim ? Object.freeze({id:claim.id,assignmentId:claim.assignment_id,planId:claim.plan_id,topicId:claim.topic_id,text:claim.claim_text,type:claim.claim_type,lifecycle:claim.lifecycle,revision:claim.revision,rowVersion:claim.row_version}) : null, activeEvidence: evidence ? Object.freeze({id:evidence.id,assignmentId:evidence.assignment_id,planId:evidence.plan_id,topicId:evidence.topic_id,sourceObjectType:evidence.source_object_type,sourceObjectId:evidence.source_object_id,locationType:evidence.location_type,reviewState:evidence.review_state,rowVersion:evidence.row_version}) : null, activeCompositionPlan: compositionPlan ? Object.freeze({id:compositionPlan.id,assignmentId:compositionPlan.assignment_id,researchPlanId:compositionPlan.research_plan_id,lifecycle:compositionPlan.lifecycle,revision:compositionPlan.revision,rowVersion:compositionPlan.row_version}) : null, activeCompositionSection: compositionSection ? Object.freeze({id:compositionSection.id,planId:compositionSection.plan_id,assignmentId:compositionSection.assignment_id,title:compositionSection.title,order:compositionSection.section_order,rowVersion:compositionSection.row_version}) : null, activeDraftDocument: draftDocument ? Object.freeze({id:draftDocument.id,assignmentId:draftDocument.assignment_id,compositionPlanId:draftDocument.composition_plan_id,title:draftDocument.title,currentVersionId:draftDocument.current_version_id,rowVersion:draftDocument.row_version}) : null, activeDraftVersion: draftVersion ? Object.freeze({id:draftVersion.id,draftId:draftVersion.draft_id,assignmentId:draftVersion.assignment_id,versionNumber:draftVersion.version_number,origin:draftVersion.origin,createdAt:draftVersion.created_at}) : null, activeHumanisationProfile: humanisationProfile ? Object.freeze({id:humanisationProfile.id,name:humanisationProfile.name,genre:humanisationProfile.genre,lifecycle:humanisationProfile.lifecycle,currentRevisionId:humanisationProfile.current_revision_id,rowVersion:humanisationProfile.row_version}) : null, activeHumanisationSession: humanisationSession ? Object.freeze({id:humanisationSession.id,assignmentId:humanisationSession.assignment_id,draftId:humanisationSession.draft_id,sourceDraftVersionId:humanisationSession.source_draft_version_id,profileId:humanisationSession.profile_id,state:humanisationSession.state,rowVersion:humanisationSession.row_version,updatedAt:humanisationSession.updated_at}) : null, activeLecturerReviewSession: lecturerReviewSession ? Object.freeze({id:lecturerReviewSession.id,assignmentId:lecturerReviewSession.assignment_id,draftId:lecturerReviewSession.draft_id,sourceDraftVersionId:lecturerReviewSession.source_draft_version_id,state:lecturerReviewSession.state,rowVersion:lecturerReviewSession.row_version,updatedAt:lecturerReviewSession.updated_at}) : null, activeLecturerFinding: lecturerFinding ? Object.freeze({id:lecturerFinding.id,sessionId:lecturerFinding.session_id,assignmentId:lecturerFinding.assignment_id,category:lecturerFinding.category,severity:lecturerFinding.severity,status:lecturerFinding.status,title:lecturerFinding.title,rowVersion:lecturerFinding.row_version}) : null, activeCorrectionPlan: correctionPlan ? Object.freeze({id:correctionPlan.id,assignmentId:correctionPlan.assignment_id,reviewSessionId:correctionPlan.review_session_id,sourceDraftVersionId:correctionPlan.source_draft_version_id,title:correctionPlan.title,state:correctionPlan.state,rowVersion:correctionPlan.row_version}) : null, activeCorrectionItem: correctionItem ? Object.freeze({id:correctionItem.id,planId:correctionItem.plan_id,assignmentId:correctionItem.assignment_id,title:correctionItem.title,actionType:correctionItem.action_type,state:correctionItem.state,rowVersion:correctionItem.row_version}) : null, activeCorrectionSession: correctionSession ? Object.freeze({id:correctionSession.id,assignmentId:correctionSession.assignment_id,correctionPlanId:correctionSession.correction_plan_id,sourceDraftVersionId:correctionSession.source_draft_version_id,state:correctionSession.state,rowVersion:correctionSession.row_version}) : null, originSurface: row.origin_surface || null, userPinned: Boolean(row.user_pinned), updatedAt: row.updated_at || null, assessmentClassification: assignment ? this.assignmentClassification(assignment.id) : null});
    }
    ...
    update(input = {}) {
        ...
        const timestamp = Model.now();
        this.store.transaction(() => {
            this.db.prepare(`INSERT INTO stud_working_context (id,active_course_id,active_assignment_id,active_requirement_contract_id,active_object_type,active_object_id,active_workflow_id,active_workflow_node_id,active_research_plan_id,active_research_topic_id,active_claim_id,active_evidence_id,active_composition_plan_id,active_composition_section_id,active_draft_document_id,active_draft_version_id,active_humanisation_profile_id,active_humanisation_session_id,active_lecturer_review_session_id,active_lecturer_finding_id,active_correction_plan_id,active_correction_item_id,active_correction_session_id,origin_surface,user_pinned,updated_at)
                VALUES ('current',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ON CONFLICT(id) DO UPDATE SET active_course_id=excluded.active_course_id,active_assignment_id=excluded.active_assignment_id,active_requirement_contract_id=excluded.active_requirement_contract_id,active_object_type=excluded.active_object_type,active_object_id=excluded.active_object_id,active_workflow_id=excluded.active_workflow_id,active_workflow_node_id=excluded.active_workflow_node_id,active_research_plan_id=excluded.active_research_plan_id,active_research_topic_id=excluded.active_research_topic_id,active_claim_id=excluded.active_claim_id,active_evidence_id=excluded.active_evidence_id,active_composition_plan_id=excluded.active_composition_plan_id,active_composition_section_id=excluded.active_composition_section_id,active_draft_document_id=excluded.active_draft_document_id,active_draft_version_id=excluded.active_draft_version_id,active_humanisation_profile_id=excluded.active_humanisation_profile_id,active_humanisation_session_id=excluded.active_humanisation_session_id,active_lecturer_review_session_id=excluded.active_lecturer_review_session_id,active_lecturer_finding_id=excluded.active_lecturer_finding_id,active_correction_plan_id=excluded.active_correction_plan_id,active_correction_item_id=excluded.active_correction_item_id,active_correction_session_id=excluded.active_correction_session_id,origin_surface=excluded.origin_surface,user_pinned=excluded.user_pinned,updated_at=excluded.updated_at`)
                .run(value.courseId, value.assignmentId, contract && contract.id || null, value.objectType, value.objectId, value.workflowId, value.workflowNodeId, value.researchPlanId, value.researchTopicId, value.claimId, value.evidenceId, value.compositionPlanId, value.compositionSectionId, value.draftDocumentId, value.draftVersionId, value.humanisationProfileId, value.humanisationSessionId, value.lecturerReviewSessionId, value.lecturerFindingId, value.correctionPlanId, value.correctionItemId, value.correctionSessionId, value.originSurface, value.userPinned ? 1 : 0, timestamp);
        });
        return this.read();
    }
