...
class StudCompositionService {
    ...
    // Main-process-only M11 integration. This is deliberately not wired to the
    // renderer's ordinary Draft-save IPC channel: lineage and AI origin are
    // derived from an accepted Humanisation Session, never renderer claims.
    saveHumanisationVersion(input = {}) {
        Academic.assertAllowedKeys(input,["assignmentId","draftId","sourceVersionId","sections","sessionId","profileRevisionId"],"Humanisation Draft version save");
        const assignment=this.repository.assignment(input.assignmentId),draft=this.repository.draftRow(input.draftId),source=this.repository.hydrateVersion(input.sourceVersionId);
        if(draft.assignmentId!==assignment.id||source.assignmentId!==assignment.id||source.draftId!==draft.id)throw new Academic.StudError("CROSS_DRAFT_VERSION","Humanisation source Version does not belong to this Draft.");
        if(draft.currentVersionId!==source.id)throw new Academic.StudError("SOURCE_DRAFT_CHANGED","The Draft advanced after Humanisation began. Review a new Session instead of applying to stale text.");
        if(!Array.isArray(input.sections)||input.sections.length>Domain.LIMITS.sections)throw new Academic.StudError("INVALID_INPUT","Humanisation Sections must be bounded.");
        const plan=this.repository.hydratePlan(draft.compositionPlanId),allowed=new Map(plan.sections.map(section=>[section.id,section])),base=new Map(source.sections.map(item=>[item.sectionId,item.content]));
        input.sections.forEach(item=>{Academic.assertAllowedKeys(item,["sectionId","content"],"Humanisation accepted Section");const id=Academic.safeId(item.sectionId,"Composition Section ID");if(!allowed.has(id))throw new Academic.StudError("CROSS_PLAN_SECTION","Accepted Section does not belong to the Draft Plan.");base.set(id,Domain.normalizeDraftContent(item.content));});
        const snapshots=plan.sections.map(section=>{const content=base.get(section.id)||"";return {sectionId:section.id,content,contentHash:Domain.canonicalHash(content),measuredLength:Domain.measuredLength(content,section.lengthUnit)};}),totalCharacters=snapshots.reduce((sum,item)=>sum+item.content.length,0);
        if(totalCharacters>Domain.LIMITS.draftTotalCharacters)throw new Academic.StudError("PAYLOAD_TOO_LARGE","Accepted Humanisation exceeds the Draft bound.");
        const versionNumber=Number(this.repository.db.prepare("SELECT MAX(version_number) version FROM stud_draft_versions WHERE draft_id=?").get(draft.id).version||0)+1,totalLength=snapshots.reduce((sum,item)=>sum+(allowed.get(item.sectionId).lengthUnit===plan.lengthUnit?item.measuredLength:0),0),contentHash=Domain.canonicalHash({draftId:draft.id,planId:plan.id,planHash:plan.planHash,versionNumber,sections:snapshots.map(item=>({sectionId:item.sectionId,contentHash:item.contentHash}))});
        return this.repository.insertVersion(draft,{versionNumber,parentVersionId:source.id,origin:"LOCAL_AI",changeReason:"Explicitly accepted Humanisation candidate.",contentHash,totalLength,lengthUnit:plan.lengthUnit,humanisationSessionId:Academic.safeId(input.sessionId,"Humanisation Session ID"),humanisationProfileRevisionId:Academic.safeId(input.profileRevisionId,"Profile Revision ID")},snapshots);
    }

    // Main-process-only M12 integration. The renderer cannot declare model
    // origin or lineage. Only an accepted Correction Session reaches here.
    saveCorrectionVersion(input = {}) {
        Academic.assertAllowedKeys(input,["assignmentId","draftId","sourceVersionId","sections","reviewSessionId","correctionPlanId","correctionSessionId","origin"],"Correction Draft version save");
        const assignment=this.repository.assignment(input.assignmentId),draft=this.repository.draftRow(input.draftId),source=this.repository.hydrateVersion(input.sourceVersionId);
        if(draft.assignmentId!==assignment.id||source.assignmentId!==assignment.id||source.draftId!==draft.id)throw new Academic.StudError("CROSS_DRAFT_VERSION","Correction source Version does not belong to this Draft.");
        if(draft.currentVersionId!==source.id)throw new Academic.StudError("SOURCE_DRAFT_CHANGED","The Draft advanced after correction began. Review a new Correction Session instead of applying stale text.");
        if(!Array.isArray(input.sections)||!input.sections.length||input.sections.length>Domain.LIMITS.sections)throw new Academic.StudError("INVALID_INPUT","Accepted Correction Sections must be bounded.");
        const plan=this.repository.hydratePlan(draft.compositionPlanId),allowed=new Map(plan.sections.map(section=>[section.id,section])),base=new Map(source.sections.map(item=>[item.sectionId,item.content]));
        input.sections.forEach(item=>{Academic.assertAllowedKeys(item,["sectionId","content"],"Accepted Correction Section");const id=Academic.safeId(item.sectionId,"Composition Section ID");if(!allowed.has(id))throw new Academic.StudError("CROSS_PLAN_SECTION","Accepted Correction Section does not belong to the Draft Plan.");base.set(id,Domain.normalizeDraftContent(item.content));});
        const snapshots=plan.sections.map(section=>{const content=base.get(section.id)||"";return {sectionId:section.id,content,contentHash:Domain.canonicalHash(content),measuredLength:Domain.measuredLength(content,section.lengthUnit)};}),totalCharacters=snapshots.reduce((sum,item)=>sum+item.content.length,0);
        if(totalCharacters>Domain.LIMITS.draftTotalCharacters)throw new Academic.StudError("PAYLOAD_TOO_LARGE","Accepted correction exceeds the Draft bound.");
        const versionNumber=Number(this.repository.db.prepare("SELECT MAX(version_number) version FROM stud_draft_versions WHERE draft_id=?").get(draft.id).version||0)+1,totalLength=snapshots.reduce((sum,item)=>sum+(allowed.get(item.sectionId).lengthUnit===plan.lengthUnit?item.measuredLength:0),0),contentHash=Domain.canonicalHash({draftId:draft.id,planId:plan.id,planHash:plan.planHash,versionNumber,sections:snapshots.map(item=>({sectionId:item.sectionId,contentHash:item.contentHash}))});
        return this.repository.insertVersion(draft,{versionNumber,parentVersionId:source.id,origin:input.origin==="USER"?"REVISION":"LOCAL_AI",changeReason:"Explicitly accepted Lecturer Committee correction candidate.",contentHash,totalLength,lengthUnit:plan.lengthUnit,lecturerReviewSessionId:Academic.safeId(input.reviewSessionId,"Lecturer Review Session ID"),correctionPlanId:Academic.safeId(input.correctionPlanId,"Correction Plan ID"),correctionSessionId:Academic.safeId(input.correctionSessionId,"Correction Session ID")},snapshots);
    }
