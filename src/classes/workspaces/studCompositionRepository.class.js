...
class StudCompositionRepository {
    ...
    insertVersion(draft, value, sectionContents) {
        ...
        this.db.prepare(`INSERT INTO stud_draft_versions (id,draft_id,assignment_id,version_number,parent_version_id,origin,change_reason,content_hash,total_length,length_unit,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
            .run(id,draft.id,draft.assignmentId,value.versionNumber,value.parentVersionId,value.origin,value.changeReason,value.contentHash,value.totalLength,value.lengthUnit,timestamp);
    ...
    hydrateDraft(id, options = {}) {
        ...
        const history = Object.freeze(this.db.prepare("SELECT id,draft_id,assignment_id,version_number,parent_version_id,origin,change_reason,content_hash,total_length,length_unit,created_at FROM stud_draft_versions WHERE draft_id=? ORDER BY version_number DESC LIMIT ?").all(draft.id,limit).map(row => Object.freeze(camel(row))));
