...
function fixture() {
    ...
    cc.state.schema={version:17};cc.state.error=null;cc.state.courses=[course,historic,unknown];cc.state.assignments=[assignment];cc.state.classifications=new Map([[assignment.id,{assignmentId:assignment.id,classification:'COURSEWORK',sourceKind:'DETERMINISTIC'}]]);cc.state.organisation={years:[{academicYear:'2025/26',current:true,unknown:false,terms:[{academicTerm:'Term 2',unknown:false,courses:[{...course,assignments:[assignment]}]}]},{academicYear:'2024/25',current:false,unknown:false,terms:[{academicTerm:'Term 1',unknown:false,courses:[{...historic,assignments:[]}]}]},{academicYear:'UNCLASSIFIED',current:false,unknown:true,terms:[{academicTerm:'UNCLASSIFIED',unknown:true,courses:[{...unknown,assignments:[]}]}]}],unassignedAssignments:[]};cc.state.workingContext={status:'ACTIVE',activeCourse:course,activeAssignment:assignment,activeRequirementContract:null,activeObject:{...document,entityType:'ACADEMIC_DOCUMENT'},activeWorkflow:null,activeWorkflowNode:null,originSurface:'DOCUMENTS',userPinned:true,updatedAt:now};cc.state.selectedCourseId=course.id;cc.state.selectedAssignmentId=assignment.id;cc.state.courseContext={course,assignments:[assignment],resources:[],notes:[],revisions:[],papers:[],references:[],provenance:[]};cc.state.assignmentContext={course,assignment,documents:[document],resources:[],notes:[],papers:[],revisions:[],links:[],references:[],provenance:[],conflicts:[],requirements:[],requirementsContract:{assignmentId:assignment.id,current:null,draft:{lifecycle:'DRAFT',completeness:'INCOMPLETE',reviewCondition:'NEEDS_REVIEW',items:[],candidates:[],coverage:{linkedDocuments:1,inspectedDocuments:1,ocrRequiredDocuments:0,chunksInspected:1,candidatesGenerated:0,truncationReached:false},freshness:{reviewCondition:'NEEDS_REVIEW',details:[]}},history:[]},workflowState:{assignmentId:assignment.id,current:null,history:[],setup:{templates:[],suggestions:[]},contractState:{current:null,draft:null,history:[]}}};cc.state.overview={today:[],upcoming:[assignment],priority:[assignment],continue:[{type:'ASSIGNMENT',course,assignment,context:cc.state.workingContext}],moduleStatus:[{...course,activeAssignmentCount:1,nearestDueDate:assignment.dueDate}],attention:[],generatedAt:now};cc.setActiveView('MODULES');cc.render();return true})()`;
}
...
(async()=>{
    ...
    try {
        ...
        const valid=report.available&&report.context&&report.hierarchy&&report.years===3&&report.unclassified&&!report.escaped&&!report.overflow;
