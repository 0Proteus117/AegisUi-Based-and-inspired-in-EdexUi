...
(async()=>{const root=fs.mkdtempSync(path.join(os.tmpdir(),"aegis-stud-m13-"));try{
    ...
    await check("SCHEMA_V26_FRESH_NO_FABRICATED_EXECUTION",async()=>{assert.strictEqual(env.store.schemaInfo().version,26);for(const table of ["stud_execution_plans","stud_execution_steps","stud_execution_attempts","stud_model_inventory","stud_watchdog_incidents"])assert.strictEqual(env.store.db.prepare(`SELECT COUNT(*) count FROM ${table}`).get().count,0);assert.strictEqual(env.coordinator.profiles().length,3);});
    let preflight;
