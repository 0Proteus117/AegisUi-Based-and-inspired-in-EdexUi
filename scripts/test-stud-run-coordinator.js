...
(async()=>{const root=fs.mkdtempSync(path.join(os.tmpdir(),"aegis-stud-m13-"));try{
    ...
    let preflight;
    await check("M14_STORAGE_RUN_IS_VISIBLE_WITHOUT_EXECUTION_PLAN",async()=>{const storageCase=fixture(env,"Synthetic storage-only work"),created=env.artifacts.createRun({assignmentId:storageCase.assignment.id,operationType:"STORAGE_TRANSFER",actor:"USER",progressMode:"INDETERMINATE",canPause:false,canCancel:true}),run=env.artifacts.transitionRun({runId:created.id,expectedVersion:created.rowVersion,action:"START"});const mission=env.coordinator.missionState({assignmentId:storageCase.assignment.id});assert.strictEqual(mission.executionPlan,null);assert.strictEqual(mission.resting,false);assert.strictEqual(mission.activeRuns[0].id,run.id);env.artifacts.transitionRun({runId:run.id,expectedVersion:run.rowVersion,action:"COMPLETE"});});
