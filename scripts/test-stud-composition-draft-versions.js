...
try{
    ...
    check("SCHEMA_V23_FRESH_WITH_NO_FABRICATED_COMPOSITION",()=>{assert.strictEqual(env.store.schemaInfo().version, 27);["stud_composition_plans","stud_composition_sections","stud_draft_documents","stud_draft_versions"].forEach(table=>assert.strictEqual(env.store.db.prepare(`SELECT COUNT(*) count FROM ${table}`).get().count,0));});
