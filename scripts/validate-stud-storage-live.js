...
function install(scenario){
 ...
 if(!["empty","resting","offline"].includes(scenario))view.setManifest({id:"manifest_synthetic",state:scenario.startsWith("cleanup")?"APPLIED":({prepared:"PREPARED",active:"COPYING",indeterminate:"COPYING",failed:"FAILED",applied:"APPLIED",rollback:"APPLIED",details:"PREPARED",long:"PREPARED"})[scenario],rowVersion:1,totalItems:files.length,totalBytes:10*1024**2,sources:[],items:files.map(file=>({reference:file.reference,byteSize:file.declaredBytes,sha256:"a".repeat(64)})),reviewIssues:[{code:"OCR_REQUIRED"}],issueCount:1,omittedFileCount:2,sharedReferenceCount:1,nextOffset:null,
 ...
 if(scenario.startsWith("cleanup")){
    view.state.manifest.retainedCopies=files.map(file=>({reference:file.reference,profileLabel:"On this Mac",classification:scenario==="cleanup-removed"?"REMOVED":"RETAINED_REQUIRES_VERIFICATION",eligibleForVerification:scenario!=="cleanup-removed",expectedAssetVersion:2}));
    view.state.manifest.cleanupHistory=scenario==="cleanup"?[]:[{state:scenario==="cleanup-active"?"VERIFYING":scenario==="cleanup-removed"?"REMOVED":"INTERRUPTED",createdAt:"2026-09-15T10:00:00.000Z",errorCode:scenario==="cleanup-interrupted"?"STORAGE_CLEANUP_REVIEW_REQUIRED":null}];
 }
 render();if(scenario==="details")root.querySelector('[data-storage-disclosure="verification"]').open=true;
 if(scenario.startsWith("cleanup"))root.querySelector('[data-storage-disclosure="cleanup"]').open=true;
...
(async()=>{
 ...
 try{
    ...
    for(const [width,height,scale] of [[1680,1050,2],[1440,900,2],[1200,780,1]])for(const appearance of ["dark","light","system-dark","system-light"]){
      ...
      for(const scenario of ["empty","resting","prepared","active","indeterminate","failed","applied","rollback","offline","details","long","cleanup","cleanup-active","cleanup-removed","cleanup-interrupted"]){
        ...
        if(output&&((width===1440&&["dark","light"].includes(appearance)&&["prepared","active","cleanup"].includes(scenario))||(width===1200&&appearance==="light"&&scenario==="details"))){const screenshot=await send("Page.captureScreenshot",{format:"png",captureBeyondViewport:false});fs.mkdirSync(output,{recursive:true});fs.writeFileSync(path.join(output,`${width}-${appearance}-${scenario}.png`),Buffer.from(screenshot.data,"base64"));}
      }
    }
