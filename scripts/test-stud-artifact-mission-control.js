...
try {
    ...
    check("CURRENT_SCHEMA_AND_NO_FABRICATED_OPERATIONAL_STATE", () => {
        assert.strictEqual(store.schemaInfo().version, 21);
    ...
    check("V18_TO_V19_MIGRATION_PRESERVES_ASSIGNMENT_WITHOUT_FABRICATION", () => {
        assert.strictEqual(migration.store.schemaInfo().version, 21); assert.ok(migration.store.getEntity("ASSIGNMENT", legacyAssignment.id));
