...
try {
    ...
    const freshRoot = path.join(root, "fresh-v13");
    const fresh = freshStore(freshRoot);
    check("FRESH_DATABASE_REACHES_SCHEMA_V13", () => assert.strictEqual(fresh.schemaInfo().version, 13));
    ...
    removeTables(v12, ["stud_discipline_profile", "stud_tool_preferences"]);
    v12.db.prepare("DELETE FROM stud_schema_migrations WHERE version=13").run();
    v12.close();
    const upgradedV12 = freshStore(v12Root);
    check("MIGRATES_REPRESENTATIVE_V12_TO_V13", () => {
        assert.strictEqual(upgradedV12.schemaInfo().version, 13);
    ...
    removeTables(v9, ["stud_discipline_profile", "stud_tool_preferences", "stud_repository_references", "stud_datasets", "stud_notebook_outputs", "stud_notebook_cells", "stud_notebooks", "stud_context_packages", "stud_context_decisions", "stud_concept_observations", "stud_academic_concepts"]);
    ...
    check("MIGRATES_REPRESENTATIVE_V9_TO_V13_WITH_DATA", () => {
        assert.strictEqual(upgradedV9.schemaInfo().version, 13);
