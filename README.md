...
![Version](https://img.shields.io/badge/version-2.5.7-13263A)
...
the ephemeral session by default or sends case data to a provider.
  v2.5.7 adds explicit Investigation Orchestration and Case Overview: a
  bounded, local context handoff layer ties existing OSINT capabilities to the
  active Case without automatic provider chaining, hidden persistence or a
  second investigation database. v2.5.6 adds local, analyst-controlled Entity Resolution: provenance-aware
